...
# CME FedWatch historical probabilities reported for 2026-07-09.
# These are the exact rows from the July 29 and September 16 meeting CSVs.
# Buckets are FOMC target ranges; midpoints are used for numeric work.
cme_july <- tribble(
  ~bucket,        ~midpoint, ~prob,
  "3.50–3.75%", 3.625,  0.754138,
  "3.75–4.00%", 3.875,  0.245862
)

cme_sept <- tribble(
  ~bucket,        ~midpoint, ~prob,
  "3.50–3.75%", 3.625,  0.376530,
  "3.75–4.00%", 3.875,  0.500363,
  "4.00–4.25%", 4.125,  0.123107
)
```
...
:::: {.Exercise #exercise-overlapping-events}
## Overlapping and mutually exclusive events

For one e-commerce order, the probability of a return is 0.18, the
probability of a customer-service contact is 0.24, and the probability of both
is 0.09. Find the probability that the order is returned or produces a
customer-service contact. Why would adding 0.18 and 0.24 give the wrong answer?

A separate order must use exactly one of two shipping methods: standard or
express. Their probabilities are 0.68 and 0.32. Find the probability that the
order uses standard or express shipping, and explain why no overlap correction
is needed.

::: {#solution-overlapping-events .details .solution-details summary="Solution"
display="show" non-interactive-summary="Solution"}

Returns and customer-service contacts can overlap, so the addition rule gives

$$
0.18+0.24-0.09=0.33.
$$

Adding only the first two probabilities would count the 0.09 overlap twice.
Standard and express shipping are mutually exclusive and cover the two
possible methods, so their union has probability $0.68+0.32=1$.
...
:::: {.Exercise #exercise-workplace-random-variables}
## Three workplace outcomes

The Bloom hybrid-work study records whether an employee leaves the firm, the
number of lines of code an engineer writes in a day, and the time an employee
spends commuting. Define a random variable for each corresponding future
outcome and classify it as discrete or continuous. Why does the count remain
discrete even when it has many possible values?

::: {#solution-workplace-random-variables .details .solution-details summary="Solution"
display="show" non-interactive-summary="Solution"}

Let $L=1$ if an employee leaves and $L=0$ otherwise. This binary random
variable is discrete. Let $C$ be the number of lines of code written in one
day. It is also discrete because its possible values form a list of counts:
$0,1,2,\ldots$. Let $T$ be commute time. We model it as continuous because it
can take any value in a range rather than only distinct counts.
...
:::: {.Exercise #exercise-support-escalation-pmf}
## Complete a support-ticket distribution

The bars show the probability distribution of $X$, the number of times a
support ticket is escalated. Complete the missing bar.

```{r}
#| label: fig-support-pmf-prompt
#| fig-cap: "Three probabilities in the support-ticket PMF. Draw the fourth bar so that the completed picture is a probability distribution."
#| fig-width: 7
#| fig-height: 3.8
#| echo: false
support_pmf <- tibble(
...
ggplot(filter(support_pmf, escalations < 3),
  ...
  annotate("text", x = 4, y = 0.08, label = "?", size = 7) +
...
1. Which bar represents no escalation? Which bars together represent at least
   one escalation?
2. What two visual checks confirm that the completed picture is a valid PMF?

::: {#solution-support-escalation-pmf .details .solution-details summary="Solution"
display="show" non-interactive-summary="Solution"}

```{r}
#| label: fig-support-pmf-solution
#| fig-cap: "The completed PMF. The leftmost bar is no escalation; the other three bars together are at least one escalation."
#| fig-width: 7
#| fig-height: 3.8
#| echo: false
ggplot(support_pmf,
...
The missing bar has height 0.05 because the four bars must sum to one. The
leftmost bar has height 0.52, so the other three bars together have probability
$1-0.52=0.48$. Every bar is nonnegative and their heights sum to one.
...
:::: {.Exercise #exercise-support-escalation-cdf}
## Accumulate the support-ticket distribution

The left panel repeats the completed PMF. On the empty axes, draw the CDF by
accumulating the bar heights from left to right.

```{r}
#| label: fig-support-cdf-prompt
#| fig-cap: "Left: the completed PMF for the number of escalations. Right: axes on which to draw its cumulative distribution function."
#| fig-width: 9
#| fig-height: 3.8
#| echo: false
...
p_support_cdf_blank <- ggplot() +
  ...
  labs(x = "Threshold, x", y = "Accumulated probability",
       title = "CDF: Pr(X ≤ x)")

p_support_pmf | p_support_cdf_blank
```

1. Circle the CDF height that gives the probability of at most one escalation.
2. Mark the jump at two escalations. How does its height recover the probability
   of exactly two escalations?

::: {#solution-support-escalation-cdf .details .solution-details summary="Solution"
display="show" non-interactive-summary="Solution"}

```{r}
#| label: fig-support-cdf-solution
#| fig-cap: "The PMF and CDF of the number of escalations. Each CDF jump has the same height as the corresponding PMF bar."
#| fig-width: 9
#| fig-height: 3.8
#| echo: false
...
p_support_cdf <- ggplot(support_cdf,
  ...
  annotate("label", x = 2.25, y = 0.89, label = "jump = 0.12",
           size = 3.5, fill = "white") +
...
The CDF reaches 0.83 at one escalation, so the probability of at most one is
0.83. At two escalations it rises from 0.83 to 0.95. That jump has height
$0.95-0.83=0.12$, matching the PMF bar at two.
...
:::: {.Exercise #exercise-continuous-delivery-cdf}
## Areas and a continuous CDF

The PDF below describes a non-normal model for delivery time $T$, measured in
days. On a copy of the density, shade the area representing each probability
before calculating it: $\Pr(T\leq 3)$, $\Pr(2<T\leq 5)$, and $\Pr(T>5)$.

```{r}
#| label: fig-delivery-density-exercise
#| fig-cap: "A non-normal PDF for delivery time. The vertical lines mark the thresholds used in the exercise."
#| fig-alt: "A non-normal, stepped delivery-time density with vertical markers at 2, 3, and 5 days."
#| fig-width: 7
#| fig-height: 3.5
#| echo: false
delivery_pdf_exercise <- tribble(
  ~xmin, ~xmax, ~density,
  0,     2,     0.140,
  2,     3,     0.390,
  3,     5,     0.135,
...
ggplot(delivery_pdf_exercise) +
  geom_rect(aes(xmin = xmin, xmax = xmax, ymin = 0, ymax = density),
            fill = "steelblue", alpha = 0.35, color = "steelblue") +
  geom_vline(xintercept = c(2, 3, 5), linetype = "dashed", color = "grey40") +
  scale_x_continuous(breaks = c(0, 2, 3, 5, 7)) +
  scale_y_continuous(breaks = NULL) +
  labs(x = "Delivery time (days)", y = "Density")
```

The model's CDF has $F(2)=0.28$, $F(3)=0.67$, and $F(5)=0.94$. Use these
values to compute the three probabilities. Why would changing
$2<T\leq 5$ to $2\leq T<5$ leave the probability unchanged?

::: {#solution-continuous-delivery-cdf .details .solution-details summary="Solution"
display="show" non-interactive-summary="Solution"}

The first probability is the area to the left of 3, the second is the area
between 2 and 5, and the third is the area to the right of 5. The CDF gives
...
$$
\Pr(2<T\leq 5)=F(5)-F(2)=0.94-0.28=0.66,
$$
...
A continuous distribution assigns probability zero to any one exact value.
Adding or removing the endpoints 2 and 5 therefore does not change the area.

:::
::::

:::: {.Exercise #exercise-marketing-channel-union}
## Overlapping marketing channels

For one customer, a retailer's model assigns probability 0.42 to opening an
email promotion, 0.28 to viewing the matching social-media promotion, and
0.15 to doing both. Find the probability that the customer sees at least one
of the promotions. Then find the probability that the customer sees neither.

::: {#solution-marketing-channel-union .details .solution-details summary="Solution"
display="show" non-interactive-summary="Solution"}

The probability of at least one exposure is

$$
0.42+0.28-0.15=0.55.
$$

The complement rule gives a probability of $1-0.55=0.45$ that the customer
sees neither promotion.
