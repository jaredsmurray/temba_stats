...
local function process_html(el, summary, content)
  ...
  local preserved_classes = {}

  for _, class_name in ipairs(el.classes) do
    if class_name ~= "details" then
      table.insert(preserved_classes, class_name)
    end
  end
  ...
  local attrs = {
    ...
    class = #preserved_classes > 0 and table.concat(preserved_classes, " ") or nil
  }
